  ...
  */
  ...
  images.each { img ->
    ...
    sh label: 'Deploy to development', script: '''
                                ...
                                helm repo update
                                helm upgrade --atomic --install ${release} skymavis/${release} --version ${chart_ver} --namespace ${env} --set-string image.repository=${img.repo} --set-string image.tag=${img.tag} --debug --dry-run 
                                '''
